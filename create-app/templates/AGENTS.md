...
This project uses [zig-webgpu-platform](https://github.com/ttrei-org/zig-webgpu-platform) as a library dependency. The platform provides:
- **Canvas API** — 2D drawing: `fillRect`, `fillCircle`, `drawLine`, `fillPolygon`, etc.
- **Color** — RGBA colors with named constants and factory methods (`fromHex`, `rgb`, etc.)
- **AppInterface** — vtable for the update/render lifecycle
- **Cross-platform** — same code runs on native desktop (Dawn/GLFW) and web (browser WebGPU/WASM)

See the platform's API.md for the full public API reference.
